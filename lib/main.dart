...
cetakTotal(){
  total = 
    (int.parse(menu[0]["harga"]) * number[0])
  ...
  Widget cekLoading(){
    if (menu == null) {
      getData();
      return getLoading();
    } else { return mainFunction();
    }
  }
  
  Widget mainFunction() {
    return Scaffold(
      appBar: new AppBar(title: new Text("Warung Makan"),),
      ...
      itemBuilder: (context, i){
        return new Padding(
          ...
          child: new Row(
            children: <Widget>[
              new Image.asset(menu[i]["url"].toString(), height: 100, width: 100,),
              new Padding(
                ...
                child: new Container(
                  ...
                  new Column(children: <Widget>[
                    Align(
                      ...
                      child: new Text(menu[i]["nama"],textAlign: TextAlign.left,style: TextStyle(fontWeight: FontWeight.bold)),
                    ),
                    Align(
                      alignment: Alignment.centerLeft,
                      child: new Text(menu[i]["detil"],textAlign: TextAlign.left),
                    ),
                    Align(
                      alignment: Alignment.centerLeft,
                      child: new Text(menu[i]["harga"],textAlign: TextAlign.left)
                    ),
                  ],),
                ),
              ),
              SizedBox(width: 5,),
              new Container(child: new RaisedButton(onPressed: (){minus(i);}, child: new Center(child: new Text("-",),),), width: 50,),
              SizedBox(width: 10,),
              new Text(number[i].toString()),
              SizedBox(width: 10,),
              new Container(child: new RaisedButton(onPressed: (){add(i);}, child: new Center(child: new Text("+"),),), width: 50,),
            ],
          ),
        ),
        );
